...
# )
from pipelines.InstructP2P_pipeline import StableDiffusionXLInstructPix2PixPipeline
...
from utils.img_util import image_grid
...
WANDB_TABLE_COL_NAMES = ["edited_image", "training_step"]
...
def log_validation(pipeline, args, accelerator, generator, global_step, epoch, is_final_validation=False):
    ...
    # pipeline.scheduler = EDMEulerScheduler(sigma_min=0.002, sigma_max=120.0, sigma_data=1.0,
    ...
    val_img_list = args.val_image_url_or_path
    val_prompt = args.validation_prompt
    if not isinstance(val_img_list,list):
        val_img_list = [val_img_list]
    if not isinstance(val_prompt,list):
    ...
    edited_images = []
    # Run inference
    for val_img_path,val_prompt_input in zip(val_img_list,val_prompt):
        input_img = pilimage.open(val_img_path).convert("RGB")
        edited_images.append(input_img.resize((out_resolution,out_resolution)))
        for val_img_idx in range(args.num_validation_images):
            a_val_img = pipeline(
                val_prompt_input,
                image=input_img,
                height=out_resolution,
                width=out_resolution,
    ...
    output_grid_image = image_grid(edited_images,len(edited_images)//(args.num_validation_images+1),(args.num_validation_images+1))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="preset/CosXL_edit",
        help="Path to pretrained model or model identifier from huggingface.co/models.",
    )
    parser.add_argument(
        "--comments",
        type=str,
        default="This train use CosXL_edit to train remove type editing",
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="experiment/CosXL_remove",
        help="The output directory where the model predictions and checkpoints will be written.",
    )
    parser.add_argument(
        "--cache_dir",
        type=str,
        default="tmp/CosXL_remove",
...
def main():
    ...
    def preprocess_train(examples):
        ...
        # augment text prompt for remove, we use multiple words to enhance the text prompt
        remove_words = ["remove", "delete", "eliminate", "drop"]
        # Preprocess the captions.
        captions = list(examples[edit_prompt_column])
        verbs_choices = torch.rand(len(captions))
        for idx,item in enumerate(captions):
            if verbs_choices[idx]<0.3:
                captions[idx] = f"remove the {item}"
            elif verbs_choices[idx]<0.6:
                captions[idx] = f"delete the {item}"
            elif verbs_choices[idx]<0.8:
                captions[idx] = f"eliminate the {item}"
            else:
                captions[idx] = f"drop the {item}"
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("CosXL-remove", config=vars(args))
    ...
    if accelerator.is_local_main_process:
        pipeline = CosStableDiffusionXLInstructPix2PixPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
    ...
    # This is needed for EDM-style training
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.do_edm_style_training:
                    concatenated_noisy_latents = torch.cat([inp_noisy_latents, original_image_embeds], dim=1)
                else:
        ...
        if accelerator.sync_gradients:
            if args.checkpointing_steps < 0:
                ...
                epoch_gap = -args.checkpointing_steps
                if accelerator.is_main_process and current_epoch%epoch_gap==0:
                    # _before_ saving state, check if this save would set us over the `checkpoints_total_limit`
                    if args.checkpoints_total_limit is not None:
                        checkpoints = os.listdir(args.output_dir)
                        checkpoints = [d for d in checkpoints if d.startswith("checkpoint")]
    ...
    if accelerator.is_main_process:
        ...
        # pipeline.save_pretrained(os.path.join(args.output_dir,"saved_pipeline"))
