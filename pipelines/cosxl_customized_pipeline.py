...
class CosStableDiffusionXLInstructPix2PixPipeline(
    ...
    def encode_prompt(
        ...
        if output_dtype:
            prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds = prompt_embeds.to(dtype=output_dtype), negative_prompt_embeds.to(dtype=output_dtype), pooled_prompt_embeds.to(dtype=output_dtype), negative_pooled_prompt_embeds.to(dtype=output_dtype)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                scaled_latent_model_input = scaled_latent_model_input.to(dtype=self.unet.dtype)
                t = t.to(dtype=self.unet.dtype)
